...
if GUIinput:
    ...
    inputpath = "O:/Administration/02 - Økonomi og PDK/Medarbejdermapper/Kasper/Focus1 - Ad hoc opgaver/genindlæggelser og genbesøg/AKA genbesøg med specialestat/"
    #excelfile = inputpath+"BGI datatræk - AKA med specialestat - subset.xlsx"
    excelfile = inputpath+"BGI datatræk - AKA med specialestat 210801-221208.xlsx"
...
outputdata['Behandlingskontakt Speciale'] = list(df_kontakter['Behandlingskontakt Service'])
...
for aa, afsn in enumerate(unique_afsn):
    ...
    if ('SJ SLA' in afsn) or ('SJ NAE' in afsn):
        ...
        outputdata['PIspec for GI fra ' + shortnames_afsn[afsn]] = NaNlist
        ...
        for aa, PIafs in enumerate(u_PIafsnit):
            ...
            for ee, ent_ktk in enumerate(ent_cpr): # løkke over alle patientkontakter for givent CPR nummer
                ...
                if len(ent_forPIogAfsnit) > 0: # Kun check for genindlæggelser hvis CPR har primærindlæggelse på afsnittet PIafs
                    ...
                    PIspec            = np.asarray(df_kontakter['Behandlingskontakt Service'][ent_forPIogAfsnit])  # Specialer for PI kontakter

                    if len(PIoverafd) > 1: # Der burde kun være en unik overafdeling på dette tidspunkt, så advar hvis det ikke er tilfældet
                        print(' - ADVARSEL: For patientkontakt nummer '+str(ent_ktk)+' (CPR='+str(cpr)+') var der '+str(len(PIoverafd))+' forskellige overafdelinger ('+str(list(PIoverafd))+'); bruger den første')
                        #pdb.set_trace()
                    ...
                    if any((GItimeMin < diffPItider) & (diffPItider < GItimeMaks ) & (df_kontakter['Behandlingskontakt record ID'][ent_ktk] != BHKforPIogAfsnit)):
                        ...
                        if isGI == 1:
                            ...
                            df_output['PIspec for GI fra ' + shortnames_afsn[PIafs]][ent_ktk] = PIspec[ent_senestePIdia][0]
...
for bb, BHKID in enumerate(uniqueBHKID):
    ...
    if any(df_output['Genindlæggelse overafdeling optælling'][ent_BHKID_alle] == 1) & (len(ent_BHKID_udskrivning) > 0):
