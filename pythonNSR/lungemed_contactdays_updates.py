...
def load_dataframes_from_excel(verbose=True):
    ...
    df_updates = pd.concat(updates_framelist)
...
def evaluate_beddays_boxplot(datemin_mostrecent='01-01-2022',verbose=True):
    ...
    ax = seaborn.boxplot(x=xvals, y=yvals, linewidth=lthick, flierprops=flierprops,
                         color=pointcolor)#, labels=, boxprops=dict(alpha=0.5))
